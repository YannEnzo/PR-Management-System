...
public class RBTree<E extends Comparable<E>> extends BST<E> implements Iterable<E>{
    ...
    private class RBTreeIterator<E extends Comparable<E>> implements Iterator<E> {
        // Stack for in-order traversal
        private Stack<TreeNode<E>> stack = new Stack<>();

        public RBTreeIterator(TreeNode<E> root) {
            pushLeft(root);
        }
    ...
    @Override /** Override the insert method to balance the tree if necessary */
...
public RBTreeNode<E> find(E e) {
        RBTreeNode<E> current = (RBTreeNode<E>)root;
        while (current != null) {
            int cmp = e.compareTo(current.element);
            if (cmp < 0) {
                current = (RBTreeNode<E>)current.left;
            } else if (cmp > 0) {
                current = (RBTreeNode<E>)current.right;
            } else {
                return current;
            }
        }
  ...
  private void fixDoubleRed(RBTreeNode<E> u, RBTreeNode<E> v,
    ...
    if (x == null || x.isBlack()) {
        ...
        if (w.left == v && v.left == u) {
            ...
            w.left = v.right; // v.right is y3 in Figure 43.6
            v.right = w;
        }
